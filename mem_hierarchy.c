...
void print_cache(nways_cache * cache) {
  ...
  for(int i = 0; i < NUM_OF_WAYS; i++) {
    ...
    for(int j = 0; j < NUM_OF_BLOCKS_PER_WAY; j++) {
      ...
      printf(" [dirty]: %d", a_block.is_dirty);
    }
    puts("\n");
  }
  puts("CACHE ====================================================================================================");
...
// We are going to use a (4 bytes or 32 bits) address [ 21 bits tag | 5 bits index | 6 bits offset]
unsigned int get_offset (unsigned int address) {
  return address & OFFSET_MASK;
};

unsigned int get_set(unsigned int address) {
  unsigned int idx = address & INDEX_MASK;
  return (idx >> OFFSET_SIZE);
};

unsigned int get_tag(unsigned int address) {
  unsigned int tag = address & TAG_MASK;
...
// This function considers a WT/~WA or WB/WA approach
void write_byte(memory_hierarchy * hierarchy, unsigned int address, unsigned char value) {
  ...
  if (cache_hit.is) {
    ...
    (*(* hierarchy->cache->ways)[cache_hit.way_number].blocks)[set_idx].is_dirty = true;
    ...
    if(CACHE_TYPE == "WT") {
      ...
      const unsigned int oldest_way = select_oldest(hierarchy->cache, set_idx);
      dequeue(&(* hierarchy->cache->fifo)[set_idx]);
      bool is_dirty = (*(* hierarchy->cache->ways)[oldest_way].blocks)[set_idx].is_dirty;
      bool has_data = (*(* hierarchy->cache->ways)[oldest_way].blocks)[set_idx].has_data;
      if (is_dirty) {
        (* (* hierarchy).ram.content)[address] = value;
        printf("  Wrote (value: '%c') in memory [DIRTY] (address: 0x%08x)\n", value, address);
        ...
        if(!has_data){
          read_tocache(hierarchy, address, oldest_way, set_idx);
          (*(*(* hierarchy->cache->ways)[oldest_way].blocks)[set_idx].content)[offset] = value;
          (*(* hierarchy->cache->ways)[oldest_way].blocks)[set_idx].is_dirty = true;
        } else {
          (*(*(* hierarchy->cache->ways)[oldest_way].blocks)[set_idx].content)[offset] = value;
          (*(* hierarchy->cache->ways)[oldest_way].blocks)[set_idx].is_dirty = true;
        }
        printf("[HIT] write_byte: Wrote (value: '%c') in cache (tag: %d, set: %d, offset: %d)\n", value, tag, set_idx, offset);
        enqueue(&(* hierarchy->cache->fifo)[set_idx], oldest_way);
      }
    }
    (* hierarchy->cache_misses)++;
  }
